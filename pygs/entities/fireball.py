...
class Fireball():
  def __init__(self, game, pos, size, box = False, move_speed=5, cooldown = 10) -> None:
    self.pos = list(pos)
    self.game = game
    self.radius = 30
    self.size = size
    self.cycle = 15
    self.box = box
    self.rect = pygame.rect.Rect(self.pos[0] + self.size[0] // 2 - 12, self.pos[1] + self.size[1] //2 - 13 , self.radius -5 , self.radius -5)
